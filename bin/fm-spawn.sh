...
#     __PIEDITRECOVERY__ absolute path to this firstmate checkout's .pi/extensions/fm-edit-recovery.ts,
#                  loaded explicitly on every pi/pi-signed launch (secondmate AND ship/scout crew)
#                  because only the firstmate repo's own worktree auto-discovers it; a crew's task
#                  worktree is a different repo entirely and never has this file on its own
...
launch_template() {
  ...
  case "$harness" in
    ...
    pi|pi-signed)
      ...
      if [ "$kind" = secondmate ]; then
        printf '%s' ' __MODELFLAG____EFFORTFLAG__-e __PITURNEND__ -e __PIWATCH__ -e __PIEDITRECOVERY__ "$(__OPINPUT__ encode launch-brief < __BRIEF__)"'
      else
        printf '%s' ' __MODELFLAG____EFFORTFLAG__-e __PIEXT__ -e __PIEDITRECOVERY__ "$(__OPINPUT__ encode launch-brief < __BRIEF__)"'
      fi
      ;;
...
sq_pieditrecovery=$(shell_quote "$FM_ROOT/.pi/extensions/fm-edit-recovery.ts")
...
LAUNCH=${LAUNCH//__PIEDITRECOVERY__/$sq_pieditrecovery}
