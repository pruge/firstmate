...
| `/updatefirstmate` | Self-update the running firstmate and its secondmates to the latest from origin with fast-forward-only pulls, then re-read instructions and nudge secondmates; say "sync upstream" for a pull-only merge of kunchenguid/firstmate commits into main through the validated gate |
