...
The hints came from the `fm-test-timing-portable-serial-*` artifacts of green CI run [32731658785](https://github.com/kunchenguid/firstmate/actions/runs/32731658785) on 2026-08-24, where that branch's lane ran 126 scripts in 3062014 ms of serial work.
That run carried nine test scripts still unmerged on main, so every script in main's then-current serial lane had a measured hint.
Two later additions to the lane ride the default weight until the next refresh.
...
| Lane | Script count | Estimated duration |
|---|---:|---:|
| `portable-serial-1of6` | 19 | 501508 ms (~501.5 s) |
| `portable-serial-2of6` | 22 | 501499 ms (~501.5 s) |
| `portable-serial-3of6` | 19 | 501496 ms (~501.5 s) |
| `portable-serial-4of6` | 20 | 501489 ms (~501.5 s) |
| `portable-serial-5of6` | 20 | 501502 ms (~501.5 s) |
| `portable-serial-6of6` | 20 | 501490 ms (~501.5 s) |
| imbalance | | 19 ms |

The single longest script, `tests/fm-pr-check-security.test.sh` at 235902 ms, is the floor for any shard count.
...
| portable serial 1-6 | job `timeout-minutes: 15` | Each balanced shard is about eight minutes of measured script time, leaving roughly 2x hang-tripwire margin for job setup and runner-speed spread. |
