...
The universal toolchain is node, git, gh with GitHub auth via `gh auth login`, no-mistakes v1.31.2 or newer, compatible gh-axi, chrome-devtools-axi, compatible lavish-axi, compatible tasks-axi per "Backlog backend" above, compatible quota-axi, and codegraph.
...
In that list, no-mistakes runs the validation pipeline, gh-axi, chrome-devtools-axi, and lavish-axi cover GitHub, browser, and rich-review operations, and tasks-axi plus quota-axi back backlog mutations and quota-aware array dispatch. `codegraph` is also a universal required tool: it backs the per-worktree code-graph index that `fm-spawn.sh` matches to a ship/scout worktree's checked-out code, and a missing binary refuses the spawn outright (captain override, 2026-08-09); once installed, a failing init/sync or a bounded timeout stay best-effort and only warn ([`docs/architecture.md`](architecture.md#worktrees-not-branches-in-your-checkout) owns the exact spawn-time behavior).
...
`codegraph` is bootstrap-detected in the universal toolchain above; a missing `codegraph` reports `MISSING: codegraph (install: npm install -g @colbymchenry/codegraph)` and also refuses `fm-spawn.sh` itself (captain override, 2026-08-09), per [`docs/architecture.md`](architecture.md#worktrees-not-branches-in-your-checkout).
...
FM_STALE_ESCALATE_SECS=240         # idle seconds before a plain (not provably-working) stale pane escalates; stale panes whose crew is not provably working surface immediately unless they declare the pause verb
FM_STALE_ESCALATE_WORKING_SECS=1200  # idle seconds before a provably-working stale pane (an active no-mistakes run-step or busy pane) escalates; longer than FM_STALE_ESCALATE_SECS so a long-but-legitimate validation is not false-alarmed for merely sharing the plain idle-pane bound
FM_BUSY_TURN_MAX_SECS=3600         # maximum age of a busy pane's latest state/<id>.turn-ended marker, or its state/<id>.meta spawn record before any turn completes, before the same wedge escalation used for a provably-working non-busy stale (at the plain FM_STALE_ESCALATE_SECS pace, not FM_STALE_ESCALATE_WORKING_SECS) takes over; inspection-only, never an automatic interrupt or restart
...
FM_SPAWN_ALLOW_NEW_WORKTREE=   # escape hatch: acquire a fresh worktree even though fm-spawn.sh found this task's own recorded worktree with uncommitted work; the old worktree is left untouched either way
