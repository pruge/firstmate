...
Planning documents live inside the target project's clone at `projects/<project>/docs/features/<feature-slug>/`, so every planned or running feature is browsable per project instead of mixed into the Firstmate home's `data/`.
Firstmate writes them under hard rule 1's enumerated `task-planning` exception and reads them back at intake and dispatch.
At planning completion firstmate commits and pushes them into the project repository itself; the `task-planning` skill owns that versioning contract and its exact mechanics.
Runtime state (briefs, metadata, backlog) remains in the Firstmate home.
...
- **Artifact location and lifecycle.** Feature documents live at `projects/<project>/docs/features/<feature-slug>/` in the primary clone, authorized by hard rule 1's enumerated `task-planning` exception and bounded to that path. Committed and merged, they are tracked files that keep the primary clone clean for guarded fleet sync; while unlanded revisions sit there untracked they trip the sync's dirty-clone report, which is exactly the recurring `STUCK: uncommitted changes` noise committing them removes.
Teardown never touches the primary clone. `ls projects/<project>/docs/features/` is the per-project inventory of planned work; progress truth remains the backlog.
