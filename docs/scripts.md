...
| `fm-worktree-bootstrap.sh` | Fill a task worktree's untracked dev environment from this home's own project clone per the project's `.worktreeinclude` declaration |
...
| `fm-project.sh`          | One read-only door to a registered project's code and plan board, refreshing the CodeGraph index before every structural answer |
...
| `fm-kill-port.sh`        | Free local TCP ports by port number only, refusing any listener rooted outside the caller's boundary |
