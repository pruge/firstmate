...
| `fm-stow-cascade.sh`     | Enumerate a primary home's registered secondmates and each one's own startup-memory accounting for an internal `/stow` cascade |
...
| `fm-codegraph-sync-lib.sh` | Match a spawned ship/scout worktree's local CodeGraph index to its checked-out code, warning instead of blocking on failure or timeout |
