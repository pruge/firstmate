...
| `assistant-working-note` | Assistant text in an `AssistantMessageComponent` message the model did not end its response with, identified by its own `stopReason` of `toolUse`, or of `length` with tool calls present | Hidden, except when the note addresses the captain anywhere in its text (case-insensitive `Captain`/`캡틴`); such an addressed note carries the genuine reply and is never hidden. |
