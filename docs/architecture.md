...
Right after that worktree is validated, and across every backend, `fm-spawn.sh` also matches the worktree's local CodeGraph index to its checked-out code through `fm_spawn_codegraph_sync` (`bin/fm-codegraph-sync-lib.sh`): `.codegraph/` is gitignored and never travels with the worktree, so it inits when absent there and syncs otherwise, since a plain `codegraph init` does not reindex an already-initialized directory. A missing `codegraph` binary refuses the spawn outright with an actionable install command (captain override, 2026-08-09) - the one deliberate spawn gate here. Once `codegraph` is installed, the rest stays a best-effort optimization: a failing init/sync or the bounded `FM_SPAWN_CODEGRAPH_TIMEOUT`-second timeout (default 60s, unbounded when neither `timeout` nor `gtimeout` is available) only warn and let the spawn continue with no synced index.
