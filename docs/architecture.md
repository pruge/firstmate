...
On the coarse runs-list fallback the newest same-branch row decides that attribution on its own, so an older same-branch run can never supply a terminal state for a live one, and a newest row whose head this worktree cannot resolve reports `unknown` rather than falling through.
