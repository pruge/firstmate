...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is neither provably working nor within its declared-pause re-surface cadence, authenticated check output such as PR merge polling or a Relay mention, stale panes in that same neither-working-nor-paused state whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_WORKING_SECS` (a longer bound than the plain idle-pane `FM_STALE_ESCALATE_SECS`, so a long-but-legitimate no-mistakes run is not false-alarmed merely for sharing the ordinary threshold), declared external waits that remain paused past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
A window is skipped from pane staleness entirely, with no `stale:` wake at all, while its PR merge watch is genuinely armed (checked fresh each cycle via the same `fm_pr_poll_artifacts_valid` integrity check the check-sweep trusts) and `bin/fm-crew-state.sh`'s authoritative current state is `done`, since the armed poll's own `check:` wake already owns that wait; a still-working, parked, blocked, failed, or unarmed done window is never exempted.
No-verb wakes, such as `working:` notes and bare turn-ended signals, are benign in either of two cases: `bin/fm-crew-state.sh` reports positive evidence that the crew is still working (an actively running no-mistakes step attributed to that crew's current code, or an exact busy verdict from the semantic busy-state contract), or its authoritative current state is a declared `paused:` external wait still inside `FM_PAUSE_RESURFACE_SECS` of that crew's own status file.
A stale (unchanging) pane applies the identical working/paused/neither classification, so a crew that declares `paused:` for a known external wait is absorbed while idle on both paths and re-surfaced only on the longer pause cadence, rather than being treated as a possible wedge.
...
A crew's very first `paused:` declaration is absorbed by the no-verb signal path too (not just later stale polls), while away mode self-handles that routine signal and owns the later recheck.
...
On the coarse runs-list fallback the newest same-branch row decides that attribution on its own, so an older same-branch run can never supply a terminal state for a live one, and a newest row whose head this worktree cannot resolve reports nothing rather than `unknown`, letting the caller fall through to pane evidence.
During no-mistakes' `ci` monitor phase, it also reads the ci step log tail because `axi status` reports both "still waiting on checks" and "checks green, waiting on merge" as `ci,running`.
The most recent recognized ci log marker wins, so checks-green monitoring reports done while a later re-arm, failed-check, or issue marker returns the crew to working.
Only when no run is attributed to this crew does it consult semantic busy state; exact busy reports working, exact idle permits fallback to a status-log event whose verb maps to a recognized run-state, and unknown or a dead pane stays unknown instead of trusting a stale log.
...
Right after that worktree is validated, and across every backend, `fm-spawn.sh` also matches the worktree's local CodeGraph indexes to its checked-out code through `fm_spawn_codegraph_sync` (`bin/fm-codegraph-sync-lib.sh`): `.codegraph/` is gitignored and never travels with the worktree, so a fresh one has no index and a reused one carries whatever sat there before, stale or not.
The root is not assumed, because `codegraph` resolves a query path upward to the nearest enclosing `.codegraph/` but never downward: a repository whose code lives in a subdirectory keeps its index beside that code, and indexing the root there would leave that index stale while a second, unqueried one sat at the root.
So the worktree is walked for existing indexes and each is synced where it already lives, since an existing `.codegraph/` is the project's own answer about where its index belongs; only a repository with no index anywhere is init'd, at its root, as the sole defensible default for a project that has expressed no preference.
A repository holding indexes in more than one place gets all of them synced rather than one picked, which is what keeps the invariant that a spawn never leaves a worktree holding two indexes where one is stale and reachable.
A missing `codegraph` binary refuses the spawn outright with an actionable install command (captain override, 2026-08-09) - the one deliberate spawn gate here. Once `codegraph` is installed, the rest stays a best-effort optimization: a failing init/sync, a walk that could not be completed, or the bounded `FM_SPAWN_CODEGRAPH_TIMEOUT`-second timeout (default 60s, unbounded when neither `timeout` nor `gtimeout` is available) only warn and let the spawn continue with no synced index. An incomplete walk that found nothing deliberately creates nothing, since an index may exist in the part that could not be read.
Right after each init or sync, `fm_codegraph_prune_if_unindexable` removes that index again when codegraph's own just-measured node and file counts show it covers only a sliver of the subtree - zero nodes, or too small a fraction of the subtree's tracked files - so a repository or subtree dominated by unsupported languages (shell and Markdown, for instance) ends a spawn with no `.codegraph/` at all rather than one that answers almost nothing. Judged only from that measured result, never from a repository or directory name, and skipped below a minimum tracked-file floor where the ratio is noise either way (captain override, 2026-08-09).
