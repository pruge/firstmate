...
Once that row reaches the mate's resolved stall threshold, the primary appends one keyed `check` wake naming the mate, row sequence, and observed age; parent receipts and queued-key deduplication suppress repeats for the same row across watcher and handling crashes, while empty and younger queues remain silent.
The threshold resolves per mate on every poll: an explicit `FM_SECONDMATE_WAKE_STALL_SECS` environment value wins, then a pinned `config/secondmate-wake-stall-secs` file fixes it, and with neither set it adapts to that mate's own observed row-consumption latency - twice the worst of its recent samples, clamped to 60..900 seconds with the historical 60-second default until observations exist - so a slow mate's normal turn pace does not produce false wake-loop alarms while a genuine minutes-long wedge still alarms inside the cap.
[`docs/configuration.md`](configuration.md) "Secondmate wake-stall threshold" owns the knob precedence and clamp details.
