...
The branch prompt owns the verdict criteria, including its unconditional explicit-request rule and one addition the captain-etiquette escalation list has no reason to carry: an outcome only MAIN can finish because the branch's role limits forbid it - a PR ready to merge, a local-only landing ready to approve, a new task that needs spawning - reports captain, since reporting it routine would leave it actionable by nobody. Unsolicited routine outcomes remain routine sailboat notes, unchanged fleet reviews remain silent, and doubt escalates.
