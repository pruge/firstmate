...
That check exempts only rebuildable machine-local artifacts firstmate itself writes or directs, such as the `.claude/settings.local.json` it writes for claude; it neither writes nor directs muse's, so a nested muse worktree or leftover scratch is the agent's own work product and MUST be able to refuse teardown.
