...
1. **Never write to a project.**
   ...
   The only exceptions are the guarded project initialization, fleet sync, secondmate sync and inherited local-material propagation, self-update, the `task-planning` and `task-grill` skills' wayfinder/grill/spec/ticket documents under a registered project's `docs/features/`, and approved `local-only` merge paths, each owned by its referenced skill or script, plus a concrete captain-approved project operation governed directly by this rule.
