...
1. **Never write to a project.**
   ...
   The only exceptions are the guarded project initialization, fleet sync, secondmate sync and inherited local-material propagation, self-update, the `task-planning`, `task-grill`, and `task-design` skills' wayfinder/grill/spec/ticket/design documents under a registered project's `docs/features/`, firstmate's own commit and push of exactly those planning documents at planning completion, and approved `local-only` merge paths, each owned by its referenced skill or script, plus a concrete captain-approved project operation governed directly by this rule.
...
4. **Ordinary crewmates never address the captain; a registered secondmate may report its own domain's results directly to the captain in its own window.**
   Ordinary crewmate communication flows through firstmate.
   A secondmate's direct report is an addition, never a replacement: the same fact must still be appended to its status file, which stays firstmate's only source of fleet truth.
...
config/secondmate-wake-stall-secs  optional pinned seconds before an unhandled local secondmate wake-queue row notifies the primary; LOCAL, gitignored, and not inherited; absent = explicit FM_SECONDMATE_WAKE_STALL_SECS env, then a per-mate adaptive threshold learned from observed row-consumption latency; see docs/configuration.md "Secondmate wake-stall threshold"
...
Keep `local-only` work in the main home.
Send in-scope work to the fitting secondmate unless it is blocked or the captain explicitly redirects it; do not read the secondmate's chat because its window is the captain's channel under hard rule 4, while routed replies return through its status or referenced document.
...
A secondmate's routed reply returns through status or a document pointer, not by firstmate peeking into its chat; the captain alone reads that window, including a secondmate's direct domain report under hard rule 4.
...
When the captain invokes `/updatefirstmate`, asks to update firstmate, or says "sync upstream" (pull-only upstream sync from `kunchenguid/firstmate`), load the `/updatefirstmate` skill.
...
- `project-management` - load before adding, creating, removing, or initializing a project.
  Cloning or registering a project is add intake and uses the same trigger.
- `task-planning` - load at ship-task intake when authorized work spans multiple meaningful changes or crews, or when unresolved decisions could materially change what is built; it owns wayfinder/spec/ticket documents under the target project's `docs/features/<feature-slug>/`, requires the ticket-to-code contrast review plus captain approval of the ticket graph before any implementation dispatch, and then hands each ready ticket verbatim to the ordinary brief/spawn lifecycle.
