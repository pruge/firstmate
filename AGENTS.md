...
- `project-management` - load before adding, creating, removing, or initializing a project.
  Cloning or registering a project is add intake and uses the same trigger.
- `task-planning` - load at ship-task intake when authorized work spans multiple meaningful changes or crews, or when unresolved decisions could materially change what is built; it owns wayfinder/spec/ticket artifacts under `data/<task-id>/plan/`, requires captain approval of the ticket graph before any implementation dispatch, and then hands each ready ticket to the ordinary brief/spawn lifecycle.
